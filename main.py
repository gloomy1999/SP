...
def main():
    ...
    nc = data_info['all_att'][...].shape[0] # 种类
    sf_size = data_info['all_att'][...].shape[1]    # 属性个数
    ...
    args.sf = semantic_data['all_att']  # 种类-属性矩阵
    # adj
    adj = adj_matrix(nc)    # 种类的邻接矩阵？Anyway，这个没用过
    args.adj = adj
    ...
    ''' optimizer '''
    odr_params = [v for k, v in model.named_parameters() if 'odr_' in k]    # AMSE模块
    zsr_params = [v for k, v in model.named_parameters() if 'zsr_' in k]    # ASVE模块
    ...
    train_transforms, val_transforms = preprocess_strategy(args.data,args)  #对图像做随机变化
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        adjust_learning_rate(optimizer, odr_optimizer, zsr_optimizer, epoch, args)  #10-kt衰减
